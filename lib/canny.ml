...
let noise_reduction_gray ~image ~mask_size =
  ...
  let resimage = Image.v ty gray heigth width in
  let apply_convolution x y =
    ...
    Image.set_pixel resimage x y (Pixel.v gray [ current_data ])
  in
...
(** Prend en entrée une image au format Gray et renvoie une carte de ses gradients en fonction de la value *)
let gradient_map_gray ?(gradient_floor = 0.)
    (image : (_, _, Color.gray) Image.t) =
  let ty = Image.ty image in
  let heigth, width, _ = Image.shape image in
  let gradient_image = Image.v ty rgb heigth width in
  let gradient_x x y =
    let left_value =
      try Pixel.get (Image.get_pixel image (x - 1) y) 0 with _ -> 0.
    in
    let right_value =
      try Pixel.get (Image.get_pixel image (x + 1) y) 0 with _ -> 0.
    in
  ...
  let gradient_y x y =
    let up_value =
      try Pixel.get (Image.get_pixel image x (y + 1)) 0 with _ -> 0.
    in
    let down_value =
      try Pixel.get (Image.get_pixel image x (y - 1)) 0 with _ -> 0.
    in
